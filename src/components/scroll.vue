<template>
    <div class="my-scroll" :class="[scrollState ? 'prohibit' : 'allow']" ref="myScroll"
        @scroll.passive="onScroll($event)" @touchstart="touchStart($event)" 
        @touchmove="touchMove($event)" @touchend="touchEnd($enent)">
...
export default {
    ...
    methods: {
        ...
        onScroll(e) {
            ...
            if(this.getScollTop) {
                this.getScollTop(e.target.scrollTop);//返回X, Y
            }
        },
        topCallback(){//刷新回调
    ...
    mounted() {
        this.myScroll = this.$ref.myScroll;//获取滑条dom
